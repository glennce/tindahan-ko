...
import { Eye, Wallet, Smartphone, Receipt, Download } from 'lucide-react';
...
export default function Shift() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-on-surface mb-1">Cash Drawer & Expenses</h1>
      <p className="text-on-surface-variant mb-6">All sales from counted days — today's sales added after you count actual cash. Expenses deduct from previous counted total.</p>

      {/* KPI Cards — Only counted days (yesterday & before) */}
        ...
        <div className="bg-surface border border-outline-variant rounded-xl p-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* <p className="text-xs text-on-surface-variant truncate">
              Counted · Cash sales ₱{Number(closedData.cash_sales ?? 0).toFixed(2)} + Payments ₱{Number(closedData.cash_utang_payments ?? 0).toFixed(2)} - Expenses ₱{Number(closedData.cash_expenses ?? 0).toFixed(2)}
            </p> */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
          ...
          {isClosed ? (
            ...
            <>
              <div className="space-y-2 text-sm mb-4">
                ...
                <div className="flex justify-between text-error"><span>Cash Expenses (today)</span><span>-₱{Number(cashExpenses).toFixed(2)}</span></div>
        ...
        <div className="bg-surface border border-outline-variant rounded-xl p-6">
          ...
          <p className="text-xs text-on-surface-variant mb-3">Deducts from previous counted total (Cash/GCash). Today's sales not affected.</p>
      ...
      {/* Expense Report — single source of truth, below expenses */}
